...
- Block type **competitor_ad_audit**: section that compares competitors using Google Ads Transparency. It supports \`discoveryMode: "local_places" | "curated_domains"\`. In the **template**, it is valid to edit \`props.heading\`, \`props.advertiserLinkLabel\` (use \`{name}\` for the competitor title), \`props.discoveryMode\`, \`props.curatedDomains\`, \`props.mapImageFit\`, and \`props.exampleImageFit\`. Block **appearance** may set link button fill (\`primaryColor\`), text on button (\`onPrimaryColor\`), heading color, and accent for labels. Use \`local_places\` for nearby-map discovery; use \`curated_domains\` for keyword-driven or national competitors and supply 3-12 real domains (optionally with \`name\`). Do not invent domains unless the user explicitly asks you to suggest or add them. Competitor rows, map images, and example creatives are still filled **per prospect** when someone runs **Run competitor audit** on the prospect page (requires a saved **service area** on that prospect; prospects may also override the curated domain list). Do not invent published competitors, map URLs, or transparency links. Do not set props.lastAuditDomainReport (legacy / internal); per-domain audit lines are stored on the job result, not in the published block.
