...
- "operations" is an ordered list of small edits. Each item MUST be a JSON object with a "type" field (never a bare string, number, or null in this array). Shape per item:
  ...
  - Block type **case_study**: props.assetId must reference a case_study content asset when assets exist. Optional overrideTitle, overrideMetric, and overrideImageUrl (block-level image URL; used by elevated split layouts when the asset has no image or the page needs a different image).
