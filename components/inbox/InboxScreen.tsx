...
export function InboxScreen({ routeThreadId }: InboxScreenProps) {
  ...
  const displayMessages = useMemo((): EmailMessage[] => {
    ...
    const pendingMessages: EmailMessage[] = threadPending.map((p) => ({
      ...
      to_email: p.toEmail,
      to_name: null,
      to_emails: p.toEmail?.trim() ? [p.toEmail.trim()] : null,
      cc: p.cc.length > 0 ? p.cc : null,
      subject: p.subject,
  ...
  const modalProps = useMemo(
    () => ({
      filters: {
        ...
        isOnSavedDefault,
        onSaveAsDefault: saveAsDefaultFilter,
        onRemoveSavedDefault: removeSavedDefaultFilter,
