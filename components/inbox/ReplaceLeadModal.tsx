...
/** Match `CustomFieldsModal.formatValue` so the read-only column renders identically. */
...
function seedStandardFields(lead: Lead | null): StandardFieldsState {
  ...
  return {
    /** Intentionally blank: the user must enter a different address than the old lead. */
    email: '',
    name: lead.name ?? '',
...
/** Trim and treat empty strings as `null` so the patch matches the DB column shape. */
...
export function ReplaceLeadModal({
  ...
  const isNarrow = width < LAYOUT_BREAKPOINT;
    ...
    try {
      ...
      if (profilePatch) {
        try {
          ...
          /** The cutover is committed; surface the partial failure but keep the inbox in sync. */
          toast.error(
            ...
            <View className="gap-3">
              {customEntries.map(([key, value]) => (
                <ComparisonRow
                  ...
                  onNewValueChange={(next) =>
                    setCustomFields((prev) => ({ ...prev, [key]: next }))
                  }
                  placeholder=""
                />
              ))}
...
function ComparisonRow({
  ...
  const showDiffHint =
    highlightDifferent && newValue.trim() !== '' && newValue.trim().toLowerCase() === (oldValue ?? '').trim().toLowerCase();
...
/**
 * Build a partial patch for the profile-only fields the RPC inherits from the old
 * lead. Only includes properties whose normalized value differs from the old lead;
 * returns `null` when nothing changed so callers can skip the follow-up update.
 */
