...
function getClientLinkPillTone(tone: ClientLinkPillTone) {
  switch (tone) {
    case 'live':
      return {
        bg: 'bg-emerald-500/10',
...
export function ClientLinkPill({ label, tone }: { label: string; tone: ClientLinkPillTone }) {
  const pillTone = getClientLinkPillTone(tone);
  return (
    <View className={`self-start rounded-full border px-2.5 py-1 ${pillTone.bg} ${pillTone.border}`}>
      <Text className={`text-xs font-instrument-medium ${pillTone.text}`}>{label}</Text>
    </View>
  );
}
...
export function PlanBadge({
  ...
  const label =
    monthlyRetainerCents != null
      ? `${preset.label} • ${formatUsd(monthlyRetainerCents)}/mo`
      : preset.label;
