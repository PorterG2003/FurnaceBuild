...
<a className="primary" href="/docs/guides/campaign-quickstart/">Campaign quickstart</a>
<a className="secondary" href="/docs/reference/">API Reference</a>
</div>

</div>

New to the API? Start with [Campaign quickstart](/docs/guides/campaign-quickstart/). Flow schemas such as [CampaignFlow](/docs/reference/schemas/CampaignFlow/) and [FlowUpdate](/docs/reference/schemas/FlowUpdate/) are in the [API Reference](/docs/reference/).

<CardGroup cols={2}>

<Card title="Authentication" icon="gear">

Send your account API key as `Authorization: Bearer f_...`. Keys are created in Furnace Account Settings. Revoked, expired, or unknown keys return `401 authentication_error`.

</Card>

<Card title="Rate limits" icon="plug">

Responses include `X-RateLimit-Limit`, `X-RateLimit-Remaining`, and `X-RateLimit-Reset`. Furnace allows 200 requests per account per minute.

</Card>

<Card title="Pagination" icon="book">

List endpoints return `{ data, limit, offset, total_count }`. Default page size is 20; maximum is 100.
...
## Idempotency
Use the `Idempotency-Key` header on `POST /v1/campaigns/{id}/leads` and `POST /v1/campaigns/{id}/leads/bulk` to safely retry imports. Furnace replays the cached response for matching account, route, key, and request-body hash for 24 hours.
...
- Sync bulk imports accept up to 100 leads per request.
- Async bulk imports accept up to 1000 leads per request and allow at most 3 queued/running jobs per account.
