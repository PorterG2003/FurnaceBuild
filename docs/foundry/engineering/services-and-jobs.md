...
| `state_matching_batch` | Amplify Step Functions `foundry-state-matching-{env}` — **parallel** `ecs:runTask.sync` for Utah + Florida + Iowa; Lambda only finalize/fail | Implemented |
