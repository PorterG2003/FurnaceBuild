...
**State matching** (mock batch + optional Utah ECS + finalize) uses **`foundryStateMatchingJob`** Lambda and **`foundry-state-matching-{env}`** Step Functions in the same file. **`POST /state-matching/batches`** creates **`foundry_jobs`** (`job_type: state_matching_batch`) and starts that state machine; **`FOUNDRY_STATE_MATCHING_STATE_MACHINE_ARN`** is injected like normalize. Utah ECS uses **worker stack** exports (cluster, subnets, SG, Utah task definition, execution + task role ARNs). Deploy **`infra/workers`** before Amplify when using Utah reconciliation.
