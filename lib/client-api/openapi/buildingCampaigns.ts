...
export function buildCampaignLaunchMarkdown(linkMode: DocLinkMode = 'openapi'): string {
  return [
    ...
    '| `paused` | Fully editable (append reactivates completed leads at former leaf nodes) | Allowed |',
    '| `stopped` | Locked — all flow edits return `flow_locked` | Blocked |',
    '',
    'Flow saves return `reactivated_count` when a paused append extends a former leaf: completed enrollments at that node are set back to `active` so they receive the new follow-up after resume.',
    '',
    '## Draft vs live lock',
    '',
