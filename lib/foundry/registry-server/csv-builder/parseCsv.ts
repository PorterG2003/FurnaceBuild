...
import type { CsvBuilderCellValue, CsvBuilderColumnDataType, CsvBuilderSourceHeaderInput } from '../../registry-types.js';
...
/**
 * Lambda Function URLs cap synchronous request bodies at 6 MB.
 * Stay comfortably under that when posting parsed rows inline.
 */
export const CSV_BUILDER_MAX_INLINE_REQUEST_BYTES = 5 * 1024 * 1024;
...
interface CsvBuilderNormalizedHeader {
  key: string;
...
function buildCsvBuilderHeaders(rawHeaders: string[]): {
...
function ensureValidUtf8(text: string): void {
  if (text.includes('\uFFFD')) {
