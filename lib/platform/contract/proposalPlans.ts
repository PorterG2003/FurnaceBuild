...
/** Reads stored plan tier only; never derives tier from billing amount. */
export function readProposalPlanTierFromSnapshot(
  snapshot: Record<string, unknown> | null | undefined,
  fallback: ProposalPlanTier = 'silver',
): ProposalPlanTier {
  if (snapshot && isProposalPlanTier(snapshot.plan_tier)) {
    return snapshot.plan_tier;
  }
  return fallback;
}
