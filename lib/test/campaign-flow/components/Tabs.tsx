...
export function Tabs({ tabs, activeTab, onTabChange, layout = 'content' }: TabsProps) {
  ...
  const snapPx = (value: number, mode: 'floor' | 'ceil' | 'round' = 'round') => {
    // On web, "device pixels" are abstracted away — what matters is snapping to whole CSS pixels.
    if (Platform.OS === 'web') {
  ...
  // Reset measurements only when the *tab IDs* change (prevents stale arrays without
  // thrashing if `tabs` is recreated every render by the parent).
  useEffect(() => {
  ...
  const computedTabWidths = useMemo(() => {
    ...
    return tabTextWidths.map((w) => {
      if (!w) return null;
      // bake horizontal padding into an integer width so DOM widths don't end up fractional
      return snapPx(w + TAB_HORIZONTAL_PADDING * 2, 'ceil');
    });
  }, [TAB_HORIZONTAL_PADDING, layout, snapPx, tabTextWidths, tabs.length]);
  ...
  // Animate indicator width to match the active tab (works even if labels cause unequal widths).
  useEffect(() => {
    const w =
      layout === 'content'
        ? computedTabWidths?.[activeTabIndex] ?? null
        : tabWidths[activeTabIndex];
    if (!w) return; // null or 0
              ...
              isEqual
                ? (event: LayoutChangeEvent) => {
                    ...
                    setTabPositions((prev) => {
                      ...
                      // Normalize x so the indicator aligns inside the padded container consistently.
                      next[index] = snapPx(Math.max(0, x - CONTAINER_PADDING), 'round');
                      return next;
                    });
                    setTabWidths((prev) => {
                      const next = prev.length === tabs.length ? [...prev] : Array(tabs.length).fill(null);
                      // Use ceil so we never under-measure and clip text; this also removes fractional widths in the DOM (web).
