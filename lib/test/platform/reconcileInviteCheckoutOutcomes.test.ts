...
function getHarnessClients() {
  ...
  if (!supabaseUrl || !serviceRoleKey || !publishableKey) {
    throw new Error('Missing Supabase credentials for reconcile invite checkout outcomes.');
  }
  return {
    supabaseUrl,
    publishableKey,
...
function createProcessingStripeClient(args: {
  invitationId: string;
  sessionId: string;
  ...
  return {
    checkout: {
      sessions: {
        retrieve: async () => ({
          ...
          invoice: `in_${args.sessionId}`,
          subscription: args.subscriptionId,
          payment_intent: {
            id: args.paymentIntentId,
            status: 'processing',
            payment_method: `pm_${args.sessionId}`,
            next_action: null,
          },
          metadata: {
            ...
            firstRecurringInvoiceAmountCents: '80000',
          },
        }),
      },
    },
    customers: {
      retrieve: async () => {
        throw new Error('customers.retrieve should not run when subscription is present');
      },
      update: async () => {
        throw new Error('customers.update should not run when subscription is present');
      },
    },
    paymentMethods: {
      retrieve: async () => {
        throw new Error('paymentMethods.retrieve should not run when subscription is present');
      },
      attach: async () => {
        throw new Error('paymentMethods.attach should not run when subscription is present');
      },
    },
    subscriptions: {
      list: async () => {
        throw new Error('subscriptions.list should not run when subscription is present');
      },
      create: async () => {
        throw new Error('subscriptions.create should not run when subscription is present');
      },
    },
    products: {
      create: async () => {
        throw new Error('products.create should not run when subscription is present');
      },
    },
    coupons: {
      create: async () => {
        throw new Error('coupons.create should not run when subscription is present');
      },
    },
  };
}

test('verification_required ACH attempt provisions from processing Stripe snapshot and stays idempotent', async (t) => {
  const namespace = createPlatformTestNamespace('reconcile-processing');
  ...
  const cleanup = {
    ...
    stripeEventIds: [] as string[],
  };

  t.after(async () => {
    if (cleanup.stripeEventIds.length) {
      await service.from('platform_invite_stripe_events').delete().in('stripe_event_id', cleanup.stripeEventIds);
    }
  ...
  const paymentIntentId = `pi_test_${namespace}`;
  const customerId = `cus_test_${namespace}`;
  const subscriptionId = `sub_test_${namespace}`;
  ...
  const stripe = createProcessingStripeClient({
  ...
  const firstEventId = `evt_test_${namespace}_1`;
  cleanup.stripeEventIds.push(firstEventId);
  const first = await reconcileInviteCheckoutSession({
    supabase: service,
    stripe,
    invitationId: invitation.id,
    paymentIntentId,
    stripeEventId: firstEventId,
    stripeEventType: 'payment_intent.processing',
  });

  assert.equal(first.provisionedNow, true);
  assert.equal(first.alreadyProvisioned, true);
  assert.equal(first.invitationStatus, 'active');
  assert.ok(first.accountId);
  cleanup.accountIds.push(first.accountId);
  ...
  const { data: invitationAfter } = await service
    .from('platform_invitations')
    .select('status, created_account_id, stripe_subscription_id')
    .eq('id', invitation.id)
    .single();
  assert.equal(invitationAfter?.status, 'active');
  assert.equal(invitationAfter?.created_account_id, first.accountId);
  assert.equal(invitationAfter?.stripe_subscription_id, subscriptionId);

  const replay = await reconcileInviteCheckoutSession({
    supabase: service,
    stripe,
    invitationId: invitation.id,
    paymentIntentId,
  ...
  assert.equal(replay.provisionedNow, false);
  assert.equal(replay.alreadyProvisioned, true);
  assert.equal(replay.accountId, first.accountId);

  const secondEventId = `evt_test_${namespace}_2`;
  cleanup.stripeEventIds.push(secondEventId);
  const second = await reconcileInviteCheckoutSession({
    supabase: service,
    stripe,
    invitationId: invitation.id,
    paymentIntentId,
    stripeEventId: secondEventId,
    stripeEventType: 'payment_intent.processing',
  });
  assert.equal(second.provisionedNow, false);
