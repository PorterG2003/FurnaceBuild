...
export function scoreDomainCandidate(companyName: string, candidate: DomainCandidate): ScoredDomain {
  ...
  const tier: ScoredDomain['tier'] = score >= 0.7 ? 'high' : score >= 0.45 ? 'medium' : 'low';
