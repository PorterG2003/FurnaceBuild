...
export function rowKey(row: Record<string, string>, keys: string[]): string {
  return keys.map((key) => (row[key] ?? '').toLowerCase()).join('|');
}
...
export function mergeDirectoryRows(
  ...
  const keep = existing.filter((row) => row.source_directory !== directory);
