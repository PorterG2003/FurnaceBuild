...
function main(): void {
  ...
  writeJson(join(runDir, 'review_summary.json'), {
    decisions: raw.decisions.length,
    qualified: rows.filter((row) => row.disposition === 'qualified').length,
    excluded: rows.filter((row) => row.disposition === 'excluded').length,
    review: rows.filter((row) => row.disposition === 'review').length,
  });
}
