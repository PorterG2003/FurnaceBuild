...
/** Meta-heavy info-product / consumer noise. */
const INFO_PRODUCT_RE =
  /\b(make money online|work from home|dropship(?:ping)?|passive income|crypto(?:currency)?|forex|binary options|weight loss|dating|onlyfans|mlm|network marketing|my (?:course|coaching)|coaching program|manifest(?:ation)?|law of attraction|slot machine|casino|sports betting)\b/i;
...
const SHORT_LINK_DOMAINS = new Set(['lnkd.in', 'bit.ly', 't.co', 'fb.me', 'l.facebook.com']);
...
function domain(url: string | null): string {
  try {
    ...
    return SHORT_LINK_DOMAINS.has(key) || host.includes('facebook.com') || host.includes('instagram.com') ? '' : key;
  } catch {
    return '';
  }
}
...
export function advertiserKey(ad: RawAd): string {
  const pageSlug = ad.advertiserUrl?.match(/facebook\.com\/([^/?#]+)/i)?.[1];
  if (pageSlug && !['ads', 'ads_library', 'www', 'pages'].includes(pageSlug.toLowerCase())) {
    return `fb:${pageSlug.toLowerCase()}`;
  }
  return domain(ad.landingUrl) ? `domain:${domain(ad.landingUrl)}` : `name:${normalizeName(ad.advertiserName)}`;
}
...
export function normalizeAndFilter(rawAds: RawAd[], config: ScrapeConfig): NormalizedAd[] {
  ...
  return [...grouped.entries()].map(([dedupeKey, ads]) => {
    ...
    const infoProduct = INFO_PRODUCT_RE.test(copy);
    ...
    const exclusionReasons = [
      ...
      ...(infoProduct ? ['info_product_or_consumer_noise'] : []),
...
export function applyReviewDecisions(ads: NormalizedAd[], decisions: ReviewDecision[]): NormalizedAd[] {
  ...
  return ads.map((ad) => {
    ...
    if (decision.decision === 'keep') return { ...ad, disposition: 'qualified', exclusionReasons: ['human_keep'] };
    if (decision.decision === 'exclude') return { ...ad, disposition: 'excluded', exclusionReasons: ['human_exclude'] };
...
export function toAdCsvRow(ad: NormalizedAd): Record<string, string> {
  return {
    ...
    ad_library_url: ad.detailUrl ?? (ad.adId ? `https://www.facebook.com/ads/library/?id=${ad.adId}` : ''),
  };
}
