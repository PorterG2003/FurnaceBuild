...
function card(ad: NormalizedAd): string {
  ...
  const detail = ad.detailUrl ? `<a href="${htmlEscape(ad.detailUrl)}" target="_blank" rel="noreferrer">Ad Library</a>` : '<span class="missing">No ad link</span>';
  ...
  <dl><dt>Auto decision</dt><dd class="auto">${htmlEscape(ad.disposition)}</dd><dt>Signals</dt><dd>${signals}</dd><dt>Reasons</dt><dd>${reasons}</dd><dt>Landing</dt><dd>${landing}</dd><dt>Ad</dt><dd>${detail}</dd></dl>
...
export function buildReviewHtml(ads: NormalizedAd[], initialDecisions: ReviewDecision[] = []): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><title>Meta webinar ad review</title>
...
<h1>Meta webinar ad review</h1><p>Review without opening Meta. Keep overrides the filter; Exclude blocks; Needs review holds it out of exports.</p>
