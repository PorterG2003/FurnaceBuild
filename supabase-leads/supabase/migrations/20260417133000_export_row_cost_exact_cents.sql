...
SELECT
  ...
  COALESCE(cre.company_enrichment_micros, 0) / 10000.0 AS company_enrichment_cost_cents,
  COALESCE(
    cre.company_enrichment_micros / NULLIF(erc.export_row_count, 0),
    0
  ) / 10000.0 AS enrichment_cost_per_row_cents,
  COALESCE(cac.company_acquisition_micros, 0) / 10000.0 AS company_acquisition_cost_cents,
  COALESCE(
...
COMMENT ON VIEW export_row_cost_summary IS
  'Owner-row export cost summary sourced from canonical cost_records with exact fractional-cent values derived from cost_amount_micros.';
...
COMMENT ON VIEW export_company_owner_leads_with_cost IS
  'export_company_owner_leads plus exact fractional-cent owner-direct and allocated company-level cost columns from cost_records.';
...
COMMENT ON VIEW export_company_owner_leads_with_contacts IS
  'export_company_owner_leads plus contact enrichment flat columns and exact fractional-cent canonical cost columns from export_row_cost_summary.';
