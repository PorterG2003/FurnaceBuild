-- ============================================
-- Migration: Replace poll_enrollments_ready with claim_enrollments_ready
-- ============================================
-- This migration replaces the SELECT FOR UPDATE approach with an atomic UPDATE-based
-- claiming mechanism that provides 100% guarantee against duplicate processing.
--
-- The function atomically claims enrollments by:
-- 1. Selecting candidate enrollments with FOR UPDATE SKIP LOCKED (for ordering)
-- 2. Updating them to mark as "processing" (sets next_run_at to future time)
-- 3. Returning the claimed enrollments
--
-- This ensures:
-- - Only one worker can successfully UPDATE each enrollment (atomic operation)
-- - WHERE clause ensures only ready enrollments are claimed
-- - If worker crashes, enrollment becomes eligible again after timeout
-- - No race conditions possible (database-level guarantee)

DROP FUNCTION IF EXISTS poll_enrollments_ready(INTEGER);
...
BEGIN
  ...
  -- Note: Using a subquery in RETURNING to avoid ambiguity with RETURNS TABLE variable names
  RETURN QUERY
