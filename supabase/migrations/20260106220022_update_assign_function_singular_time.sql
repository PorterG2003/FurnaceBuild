-- ============================================
-- Migration: Update assign_message_job_to_interval for singular time
-- ============================================
-- Changes jitter calculation to use interval_time ± (interval_duration * jitter_percentage / 2)
-- Allows negative jitter (scheduled before interval_time)
...
BEGIN
  ...
  -- If last_processed_time is NULL, this is the first interval - allow any available interval
  -- Look for intervals that are either 'available' OR 'scheduled' (if not all mailboxes have jobs yet)
  WITH next_interval AS (
    SELECT ci.id, ci.interval_time, ci.status
...
COMMENT ON FUNCTION assign_message_job_to_interval IS 'Atomically locks interval, checks mailbox, and creates message_job. Uses interval_time as base for jitter calculation. Jitter allows negative values (scheduled before interval_time). Sequential processing: only processes intervals after last processed (where all jobs are sent/failed).';
