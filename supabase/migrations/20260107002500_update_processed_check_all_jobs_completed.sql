-- ============================================
-- Migration: Processed = all eligible jobs completed (sent/failed)
-- ============================================
-- Tighten definition of a processed interval:
--   - Consider only ELIGIBLE campaign mailboxes (mailboxes.status='connected' AND smtp_status='active')
--   - Require that each eligible mailbox has at least one job in the interval
--   - Require that there are NO jobs for eligible mailboxes in non-completed states
--     (i.e. pending/reserved/sending must be zero)
...
BEGIN
  ...
  LOOP
    ...
    -- 2) There are NO non-completed jobs for eligible mailboxes in the interval
    FOR v_interval_record IN
      ...
      WHERE ci.campaign_id = v_campaign_record.id
        ...
        -- 2) Ensure there are NO non-completed jobs for eligible mailboxes
        AND NOT EXISTS (
          SELECT 1
          ...
          WHERE cm2.campaign_id = ci.campaign_id
...
COMMENT ON FUNCTION check_and_update_processed_intervals IS
  'Marks interval completed only when every eligible mailbox (connected + smtp_status=active) has at least one job in the interval and there are no non-completed jobs (pending/reserved/sending) for those mailboxes. Updates last_processed_interval_end to interval_time.';
