-- ============================================
-- Migration: Fix assign function to wait for previous interval to complete
-- ============================================
-- The issue: The scheduler was assigning jobs to new intervals before the
-- previous interval's jobs were all sent/failed. This caused intervals to
-- be processed out of order.
--
-- The fix: Only allow assigning jobs to an interval if:
-- 1) It's the FIRST 'available' or 'scheduled' interval after last_processed_interval_end
-- 2) For 'scheduled' intervals, only if this mailbox doesn't have a job there yet
--
-- This ensures strict sequential processing: all mailboxes fill the SAME interval
...
DECLARE
  ...
  v_first_available_interval_id UUID;
BEGIN
...
COMMENT ON FUNCTION assign_message_job_to_interval IS 
  'Atomically locks interval, checks mailbox, and creates message_job. Strict sequential processing: only assigns to intervals AFTER last_processed_interval_end. Multiple mailboxes can share the same interval. Uses UPDATE with subquery for correct FOR UPDATE SKIP LOCKED behavior.';
