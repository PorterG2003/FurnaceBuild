-- ============================================
-- Migration: Add p_attachments to create_inbox_reply_job and create_inbox_forward_job
-- ============================================
-- Allows attaching files when sending reply/forward. Attachments are stored in message_data
-- as JSONB array of { filename, contentType, content } (content = base64).
-- Drop the old 9-parameter overloads so the new 10-parameter versions are the only ones
-- (avoids "function name is not unique" when adding COMMENT).

DROP FUNCTION IF EXISTS create_inbox_reply_job(UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[]);
DROP FUNCTION IF EXISTS create_inbox_forward_job(UUID, UUID, UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT[]);
...
BEGIN
  ...
  -- 4. Build message_data for the worker (include attachments)
...
COMMENT ON FUNCTION create_inbox_reply_job IS
  'Creates a message_job for an inbox reply. References header is built from full thread. Optional p_attachments: JSONB array of { filename, contentType, content } (content base64).';

-- ============================================
-- create_inbox_forward_job with p_attachments
-- ============================================
...
BEGIN
  ...
  -- 3. Build message_data for the worker (no In-Reply-To/References for forward; include attachments)
  v_message_data := jsonb_build_object(
...
COMMENT ON FUNCTION create_inbox_forward_job IS
  'Creates a message_job for forwarding a message to new recipients. Forward is send-only. Optional p_attachments: JSONB array of { filename, contentType, content } (content base64).';
