-- Update check_mailbox_throttle_and_reserve to use mailbox-level throttle config
-- (min_gap_seconds, daily_limit, hourly_limit) when creating a new mailbox_throttles row.
...
BEGIN
  ...
  -- Create throttle record if doesn't exist, using mailbox-level config
  IF NOT FOUND THEN
