-- RPC: invite_user_to_account
-- Server-side check: if the invitee already has a Supabase auth account, add
-- them to the account directly; otherwise create a pending invitation row.
--
-- Returns JSONB:
--   { "status": "added" }                           – existing user, added to account
--   { "status": "already_member" }                  – user is already in the account
--   { "status": "invited", "invitation_id": "..." } – new invitation created
...
BEGIN
  ...
  IF v_target_uid IS NOT NULL THEN
    ...
    -- (handle_new_user trigger normally creates it, but be defensive).
  ...
  -- No auth account — create a pending invitation
