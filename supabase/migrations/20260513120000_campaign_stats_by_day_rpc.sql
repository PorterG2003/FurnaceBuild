-- ============================================
-- Migration: campaign_stats_by_day RPC
-- ============================================
-- Per-UTC-day aggregates for a single native campaign (matches getCampaignStatsByDay
-- semantics; avoids PostgREST row caps on raw events selects).

CREATE OR REPLACE FUNCTION public.campaign_stats_by_day(
  p_campaign_id uuid,
  p_start_date date,
  p_end_date date
)
...
COMMENT ON FUNCTION public.campaign_stats_by_day(uuid, date, date) IS
  'Campaign detail chart: one row per UTC calendar day in range with sent/replied/positive/bounce counts (Furnace campaigns only).';
