-- Rewrite account_lead_people_page to use account_lead_people rollup as the driving table.
...
COMMENT ON FUNCTION public.account_lead_people_page(uuid, text[], uuid[], text[], text[], text[], text, integer, integer, text, text) IS
  'Account-wide one-row-per-person leads explorer backed by account_lead_people rollup.';
...
-- Paginated saved list members joined to rollup rows
...
AS $$
  ...
  SELECT
    joined.global_lead_id,
    joined.email,
    joined.display_name,
    joined.first_name,
    joined.last_name,
    COALESCE(joined.campaign_count, 0),
    joined.company_list,
    COALESCE(joined.has_reply, false),
    joined.latest_activity,
    joined.newest_membership_created_at,
    COUNT(*) OVER()::bigint AS total_count
  FROM joined
  ORDER BY joined.latest_activity DESC NULLS LAST, joined.email ASC NULLS LAST
  LIMIT LEAST(GREATEST(COALESCE(p_limit, 50), 1), 500)
  OFFSET GREATEST(COALESCE(p_offset, 0), 0);
$$;

GRANT EXECUTE ON FUNCTION public.saved_lead_list_people_page(uuid, uuid, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.saved_lead_list_people_page(uuid, uuid, integer, integer) TO service_role;
