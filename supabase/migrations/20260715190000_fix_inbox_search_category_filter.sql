-- Fix list_account_inbox_threads category filter NULL trap.
-- When p_category is NULL, `p_category IN (...)` is NULL (not false), so
-- `NOT v_no_category OR t.category IS NULL` excluded every categorized thread.
-- That made body/subject FTS look broken for most inbox conversations.
...
COMMENT ON FUNCTION public.list_account_inbox_threads IS
  'List inbox threads with filters and optional FTS across subject, participants, lead, campaign, tags, and message bodies (all messages in thread).';
