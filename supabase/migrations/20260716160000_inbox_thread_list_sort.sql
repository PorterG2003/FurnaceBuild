-- Add p_sort to list_account_inbox_threads for Master Inbox sort controls.
-- Values: newest (default), open_first, oldest, unread_first.
-- Search rank remains the primary ORDER BY key when a search query is active.
...
BEGIN
  ...
  ORDER BY
    ...
    CASE WHEN v_sort <> 'oldest' THEN f.last_message_at END DESC NULLS LAST,
    f.id ASC
  LIMIT v_limit
  OFFSET v_offset;
END;
...
COMMENT ON FUNCTION public.list_account_inbox_threads IS
  'List inbox threads with filters, optional FTS, and sort (newest default, open_first, oldest, unread_first).';
