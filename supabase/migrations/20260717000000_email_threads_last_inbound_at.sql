-- Add last_inbound_at for Master Inbox triage (latest prospect reply).
-- last_message_at remains all-activity (either direction) for leads/API.

ALTER TABLE public.email_threads
  ADD COLUMN IF NOT EXISTS last_inbound_at timestamptz NULL;

COMMENT ON COLUMN public.email_threads.last_inbound_at IS
  'Latest inbound (direction = received) message time. Drives Master Inbox card time, Newest/Oldest sort, and date filters.';

UPDATE public.email_threads t
SET last_inbound_at = s.max_received_at
FROM (
  SELECT thread_id, max(received_at) AS max_received_at
  FROM public.email_messages
  WHERE direction = 'received'
  GROUP BY thread_id
) s
WHERE t.id = s.thread_id
  AND (t.last_inbound_at IS DISTINCT FROM s.max_received_at);

CREATE INDEX IF NOT EXISTS idx_email_threads_account_has_reply_status_last_inbound
  ON public.email_threads(account_id, has_reply, conversation_status, last_inbound_at DESC);

-- Recreate list RPC: sort/filter on last_inbound_at; return both timestamps.
...
DECLARE
  ...
  v_no_category boolean := v_category IN ('__no_category__', 'no_category');
  ...
  filtered AS (
    ...
    WHERE t.account_id = p_account_id
      ...
      AND (
        ...
        OR (NOT v_no_category AND t.category = v_category)
      )
      AND (
...
COMMENT ON FUNCTION public.list_account_inbox_threads IS
  'List inbox threads with filters, optional FTS, and sort. Newest/Oldest and date filters use last_inbound_at (lead reply time).';
