-- Pre-aggregate outcomes to one row per message job and copy piece before
-- computing group metadata. This avoids repeated DISTINCT JSON aggregation over
-- the event x occurrence join for accounts with thousands of events.
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance with per-job pre-aggregation, exact total reconciliation, and parse backlog.';
