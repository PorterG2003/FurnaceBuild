-- Copy renderings: the piece-set that went out for a send.
-- Stats join message_jobs.copy_rendering_id → rendering_pieces → pieces.

CREATE TABLE IF NOT EXISTS public.copy_renderings (
  ...
  UNIQUE (content_id, render_key),
  UNIQUE (id, account_id)
);

CREATE TABLE IF NOT EXISTS public.copy_rendering_pieces (
  rendering_id uuid NOT NULL REFERENCES public.copy_renderings(id) ON DELETE CASCADE,
  piece_id uuid NOT NULL REFERENCES public.copy_pieces(id) ON DELETE CASCADE,
  PRIMARY KEY (rendering_id, piece_id)
);

CREATE INDEX IF NOT EXISTS idx_copy_renderings_account_content
  ON public.copy_renderings (account_id, content_id);

CREATE INDEX IF NOT EXISTS idx_copy_rendering_pieces_piece
  ON public.copy_rendering_pieces (piece_id);

ALTER TABLE public.message_jobs
  ADD COLUMN IF NOT EXISTS copy_rendering_id uuid
  REFERENCES public.copy_renderings(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_message_jobs_copy_rendering_id
  ON public.message_jobs (copy_rendering_id)
  WHERE copy_rendering_id IS NOT NULL;

COMMENT ON TABLE public.copy_renderings IS
  'Concrete piece-set for a copy template. render_key is empty for unbranched subjects, otherwise the FNV subject-branch key.';
COMMENT ON TABLE public.copy_rendering_pieces IS
  'Pieces belonging to a copy rendering. Stats join through this table, not copy_piece_occurrences.';
COMMENT ON COLUMN public.message_jobs.copy_rendering_id IS
  'Identity FK for the copy piece-set that went out on this send. Null if copy is unparsed.';

ALTER TABLE public.copy_renderings ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.copy_rendering_pieces ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS copy_renderings_select_member ON public.copy_renderings;
CREATE POLICY copy_renderings_select_member
  ON public.copy_renderings FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.account_users au
      WHERE au.account_id = copy_renderings.account_id
        AND au.user_id = auth.uid()
    )
  );

DROP POLICY IF EXISTS copy_rendering_pieces_select_member ON public.copy_rendering_pieces;
CREATE POLICY copy_rendering_pieces_select_member
  ON public.copy_rendering_pieces FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1
      FROM public.copy_renderings cr
      JOIN public.account_users au ON au.account_id = cr.account_id
      WHERE cr.id = copy_rendering_pieces.rendering_id
        AND au.user_id = auth.uid()
    )
  );

REVOKE ALL ON TABLE
  public.copy_renderings,
  public.copy_rendering_pieces
...
DROP TABLE IF EXISTS public.copy_content_subject_branches;
DROP TABLE IF EXISTS public.copy_content_spintax_groups;
DROP FUNCTION IF EXISTS public.fnv1a32(text);
DROP FUNCTION IF EXISTS public.fnv1a32_bytea(bytea);
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance via message_jobs.copy_rendering_id, with parse backlog.';
