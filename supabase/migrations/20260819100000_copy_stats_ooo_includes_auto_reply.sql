-- Treat Auto Reply threads as OOO for copy stats (out_of_office is only set when resume is scheduled).
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance via message_jobs.copy_rendering_id; ooo_replied includes out_of_office and Auto Reply threads.';
