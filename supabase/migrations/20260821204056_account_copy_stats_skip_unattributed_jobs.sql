-- Avoid PK lookups of every message_job in the window. Authenticated role
-- statement_timeout is 8s; joining 20k+ jobs onto events was blowing it.
-- OFFSET 0 keeps the per-type LATERAL from flattening onto idx_events_created_at.
...
BEGIN
  ...
  scoped_events AS MATERIALIZED (
    ...
    CROSS JOIN LATERAL (
      SELECT e.id, e.event_type, e.event_data, e.campaign_id, e.message_job_id
      ...
      WHERE e.account_id = p_account_id
        ...
        AND (
          ...
          OR (
            e.created_at >= (p_start_date::timestamp AT TIME ZONE 'UTC')
            AND e.created_at < ((p_end_date + 1)::timestamp AT TIME ZONE 'UTC')
          )
        )
      OFFSET 0
    ) e
  ...
  piece_events AS MATERIALIZED (
    ...
    INNER JOIN attributed_jobs aj ON aj.job_id = se.job_id
    INNER JOIN public.copy_renderings cr
      ON cr.id = aj.copy_rendering_id
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance via message_jobs.copy_rendering_id; ooo_replied includes out_of_office and Auto Reply threads. Event range uses account/type/created index; jobs are joined only when copy_rendering_id is set.';
