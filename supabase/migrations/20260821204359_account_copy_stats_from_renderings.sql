-- Find attributed jobs from copy_renderings (hundreds of rows) instead of
-- scanning message_jobs by account_id. Stats currently estimate almost every
-- job has copy_rendering_id, so `WHERE copy_rendering_id IS NOT NULL` seq-scans
-- 1.3M rows and exceeds the 8s authenticated timeout.
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance via message_jobs.copy_rendering_id; jobs are found from copy_renderings, not a message_jobs account scan.';
