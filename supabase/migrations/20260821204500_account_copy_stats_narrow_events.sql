-- Materialize only event ids/types, not event_data jsonb, so the 8s API
-- timeout is not spent copying wide heap tuples for unattributed sends.
...
BEGIN
  ...
  scoped_events AS MATERIALIZED (
    ...
    CROSS JOIN LATERAL (
      SELECT e.id, e.event_type, e.campaign_id, e.message_job_id
  ...
  piece_events AS MATERIALIZED (
    SELECT
      ...
      bool_or(
        se.event_type = 'replied'
        AND COALESCE((reply_e.event_data->>'is_positive')::boolean, false)
        AND public.is_paced_campaign_message_type(aj.message_type)
    ...
    LEFT JOIN public.events reply_e
      ON reply_e.id = se.event_id
     AND se.event_type = 'replied'
