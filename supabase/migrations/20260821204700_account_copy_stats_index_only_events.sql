-- Select only columns covered by idx_events_account_type_created (INCLUDE
-- campaign_id, message_job_id) so the range is an index-only scan.
...
BEGIN
  ...
  piece_events AS MATERIALIZED (
    ...
    LEFT JOIN public.events reply_e
      ON reply_e.message_job_id = se.job_id
