-- After copy_rendering_id backfill, joining exploded pieces to events/threads
-- times out (authenticated 8s). Aggregate job outcomes first, then attach
-- pieces. Also raise the function timeout so the role 8s cap cannot cancel it.
...
BEGIN
  ...
  scoped_events AS MATERIALIZED (
    ...
    CROSS JOIN LATERAL (
      SELECT e.event_type, e.campaign_id, e.message_job_id
  ...
  job_events AS MATERIALIZED (
    ...
    INNER JOIN public.message_jobs mj ON mj.id = se.job_id
  ...
  sent_totals AS (
    ...
    FROM scoped_events se
    LEFT JOIN job_events je ON je.job_id = se.job_id
  ),
  backlog AS (
    SELECT
      (
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Account-scoped copy-piece performance. Job outcomes first, then pieces; function timeout 30s.';
