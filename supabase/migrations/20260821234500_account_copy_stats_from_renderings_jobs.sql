-- Revenue Strategy (and similar) have many unattributed sends. Looking up
-- message_jobs by every event id is the slow path. Find stamped jobs from
-- copy_renderings, join those to the date-scoped events, then attach pieces.
