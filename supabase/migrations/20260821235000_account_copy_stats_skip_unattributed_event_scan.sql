-- Do not scan every account sent event. Piece stats use events for stamped
-- jobs only; total_sent comes from campaign_stats_daily (same as the hero).
...
BEGIN
  ...
  job_events AS MATERIALIZED (
    ...
    CROSS JOIN LATERAL (
      SELECT ev.event_type, ev.campaign_id
      FROM public.events ev
      WHERE ev.message_job_id = aj.job_id
        AND ev.account_id = p_account_id
