-- Hash-join index-only account events to stamped jobs. Nested loops from
-- each job into events(message_job_id) were ~11s on Revenue Strategy.
...
BEGIN
  ...
  scoped_attributed_events AS MATERIALIZED (
    SELECT
      e.event_type,
      e.campaign_id,
    ...
    INNER JOIN attributed_jobs aj ON aj.job_id = e.message_job_id
  ),
  job_events AS MATERIALIZED (
    ...
    INNER JOIN scoped_attributed_events se ON se.job_id = aj.job_id
    GROUP BY
      aj.job_id,
      se.campaign_id,
      se.campaign_name,
