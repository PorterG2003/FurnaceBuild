...
BEGIN
  ...
  job_events AS MATERIALIZED (
    ...
    LEFT JOIN public.nodes n ON n.id = aj.node_id
    GROUP BY
      aj.job_id,
      e.campaign_id,
      cs.name,
      ...
      aj.message_type,
      aj.node_id,
      COALESCE(NULLIF(btrim(n.node_data->>'label'), ''), 'Email step')
  ),
  ...
  reply_flags AS MATERIALIZED (
    ...
    LEFT JOIN public.events reply_e
      ON reply_e.message_job_id = r.job_id
     AND reply_e.account_id = p_account_id
     AND reply_e.event_type = 'replied'
     AND reply_e.created_at >= v_start
     AND reply_e.created_at < v_end
    GROUP BY r.job_id
  ),
