-- Copy stats first-load was ~9s: random page reads of message_jobs/events.
-- Denormalize copy_rendering_id onto events and scan that slice in date order.
...
SET copy_rendering_id = mj.copy_rendering_id
FROM public.message_jobs mj
WHERE e.message_job_id = mj.id
  AND mj.copy_rendering_id IS NOT NULL
  AND e.copy_rendering_id IS NULL
  AND e.event_type IN ('sent', 'replied', 'bounced');

CREATE INDEX IF NOT EXISTS idx_events_account_created_copy_rendering
  ON public.events (account_id, created_at)
  INCLUDE (campaign_id, message_job_id, copy_rendering_id, event_type)
  WHERE copy_rendering_id IS NOT NULL
    AND event_type IN ('sent', 'replied', 'bounced');

CREATE OR REPLACE FUNCTION public.events_fill_copy_rendering_id()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  IF NEW.copy_rendering_id IS NULL AND NEW.message_job_id IS NOT NULL THEN
    SELECT mj.copy_rendering_id
      INTO NEW.copy_rendering_id
    FROM public.message_jobs mj
    WHERE mj.id = NEW.message_job_id;
  END IF;
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS events_fill_copy_rendering_id ON public.events;
CREATE TRIGGER events_fill_copy_rendering_id
  BEFORE INSERT ON public.events
  FOR EACH ROW
  WHEN (NEW.event_type IN ('sent', 'replied', 'bounced'))
  EXECUTE FUNCTION public.events_fill_copy_rendering_id();

-- Revenue Strategy last-30 still took ~12s: we materialized every sent/replied/
-- bounced event, then joined. Drive from stamped jobs (~2.7k) and take totals
-- from campaign_stats_daily so unattributed sends are never scanned.
...
BEGIN
  ...
  scoped_copy_events AS MATERIALIZED (
    SELECT
      e.event_type,
      e.campaign_id,
      ...
      e.copy_rendering_id,
      COALESCE((e.event_data->>'is_positive')::boolean, false) AS is_positive
    FROM public.events e
    INNER JOIN campaign_scope cs ON cs.id = e.campaign_id
    WHERE e.account_id = p_account_id
      AND e.copy_rendering_id IS NOT NULL
      AND e.event_type IN ('sent', 'replied', 'bounced')
  ...
  job_events AS MATERIALIZED (
    SELECT
      ...
      se.campaign_id,
      se.campaign_name,
      se.copy_rendering_id,
      NULL::uuid AS node_id,
      'Email step'::text AS node_label,
      bool_or(se.event_type = 'sent') AS was_sent,
      bool_or(se.event_type = 'replied') AS was_replied,
      bool_or(se.event_type = 'bounced') AS was_bounced,
      bool_or(se.event_type = 'replied' AND se.is_positive) AS was_positive_reply
    FROM scoped_copy_events se
    GROUP BY
      se.job_id,
      se.campaign_id,
      se.campaign_name,
  ...
  replied_job_ids AS MATERIALIZED (
    SELECT je.job_id
    FROM job_events je
  ...
  reply_flags AS MATERIALIZED (
    ...
    FROM replied_job_ids r
    LEFT JOIN public.email_threads et
      ON et.message_job_id = r.job_id
    GROUP BY r.job_id
  ),
  piece_events AS MATERIALIZED (
    SELECT
      ...
      (je.was_replied AND je.was_positive_reply) AS was_positive_reply,
      je.was_bounced
    FROM job_events je
  ...
  sent_totals AS (
    SELECT
      COALESCE(
        (
          SELECT SUM(d.sent_count)::bigint
          FROM public.campaign_stats_daily d
          INNER JOIN campaign_scope cs ON cs.id = d.campaign_id
          WHERE d.account_id = p_account_id
            AND (p_start_date IS NULL OR d.stat_date >= p_start_date)
            AND (p_end_date IS NULL OR d.stat_date <= p_end_date)
        ),
        (
          SELECT COUNT(*)::bigint
          FROM public.events e
      ...
      (
        SELECT COUNT(*) FILTER (WHERE je.was_sent)::bigint
        FROM job_events je
      ) AS attributed_sends
  ),
  backlog AS (
...
COMMENT ON FUNCTION public.account_copy_stats(uuid, date, date, uuid[], text, text) IS
  'Copy-piece performance from stamped jobs; send totals from campaign_stats_daily.';
