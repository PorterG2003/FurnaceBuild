...
export function resolveEmailStepNumber(
  flowData: FlowDataLike<FlowNodeLike> | null | undefined,
  flowNodeId: string | null | undefined,
): number | undefined {
  if (!flowNodeId) return undefined;
  const index = getEmailNodesInSendOrder(flowData).findIndex((node) => node.id === flowNodeId);
