/**
 * ECS entry: load Iowa company IDs from foundry_jobs payload, scrape Iowa SOS, persist to leads DB, reconcile.
...
async function applyOwnerOutcome(
  ...
  await updateEntityOwnerResolution(
    client as Parameters<typeof updateEntityOwnerResolution>[0],
    {
...
function scrapeFailureReason(result: Awaited<ReturnType<typeof scrapeIowaCompanyFromSearchForm>>): string {
  if (result.pick.ambiguous) return 'ambiguous_search';
  if (result.error?.trim()) return result.error.trim();
  if (!result.pick.hit) return 'no_search_hit';
  if (!result.detail) return 'parse_detail_failed';
  return 'iowa_lookup_failed';
}
...
async function main() {
  ...
  if (missingEnv.length > 0) {
    console.error(`Iowa reconciliation: missing environment variable(s): ${missingEnv.join(', ')}`);
    process.exit(1);
  }
  ...
  if (!secretKey) {
    console.error('Iowa reconciliation: LEADS_SUPABASE_SECRET_KEY is empty after SSM fetch');
    process.exit(1);
  }

  const client = createClient(leadsUrl, secretKey, {
  ...
  const payload = (jobRow.payload ?? {}) as { iowa_company_ids?: string[] };
  ...
  const companyIds = (() => {
    ...
    return payload.iowa_company_ids ?? [];
  })();
    ...
    for (let i = 0; i < companyIds.length; i++) {
      ...
      if (r.rateLimited || /iowa_rate_limit/.test(r.error ?? '')) {
        logRec('rate-limit-cooldown', {
          companyId,
          cooldownMs: postRateLimitCooldownMs,
          error: r.error ?? null,
        });
        await page.waitForTimeout(postRateLimitCooldownMs);
      }
      logRec('company-scrape-finished', {
        ...
        businessNumber: r.detail?.businessNumber ?? r.pick.hit?.businessNumber ?? null,
      });
      const lk = lookupKey(co.normalized_key as string | null, legalName);

      try {
        if (r.pick.ambiguous || !r.pick.hit || !r.detail || r.error) {
          const scrape = scrapeFailureReason(r);
          await client.from('reconciliation_results').insert({
            reconciliation_run_id: reconciliationId,
            company_id: companyId,
            outcome: 'error',
            details: {
              scrape,
              error: r.error ?? scrape,
              hit_count: r.hits.length,
              ambiguous: r.pick.ambiguous,
              picked_business_number: r.pick.hit?.businessNumber ?? null,
            },
            matcher_version: 'foundry_matcher_v1',
            scoring_version: 'foundry_score_v1',
            ruleset_version: 'foundry_rules_v1',
          ...
          await noteOutcomePersisted();
          perCompany.push({ companyId, state: 'IA', error: r.error ?? scrape });
          logRec('company-persist-skipped', { companyId, error: r.error ?? scrape });
          continue;
        }
        ...
        const rootRegistryKey = buildRegistryEntityKey('IA', r.detail.businessNumber);
        ...
        while (queue.length > 0) {
          ...
          const queryKey = buildOwnerQueryKey('IA', item.ownerNameRaw);
          ...
          const child = await scrapeIowaCompanyFromSearchForm(page, item.ownerNameRaw);
          if (child.rateLimited || /iowa_rate_limit/.test(child.error ?? '')) {
            logRec('drilldown-rate-limit-cooldown', {
              companyId,
              ownerName: item.ownerNameRaw,
              cooldownMs: postRateLimitCooldownMs,
          ...
          if (child.pick.ambiguous) {
            const outcome = {
              status: 'entity_ambiguous' as const,
              notes: { owner_name: item.ownerNameRaw, candidates: child.hits.map((h) => h.entityName).slice(0, 10) },
            };
          ...
          if (!child.detail || child.error) {
            const outcome = {
              status: 'entity_parse_failed' as const,
              notes: { owner_name: item.ownerNameRaw, error: child.error ?? 'parse_failed' },
            };
            queryOutcomes.set(queryKey, outcome);
            await applyOwnerOutcome(client, item.ownerRowId, outcome);
          ...
          const childRegistryKey = buildRegistryEntityKey('IA', child.detail.businessNumber);
          const existingStateEntityId = resolvedByRegistryKey.get(childRegistryKey);
          if (existingStateEntityId) {
            const outcome = {
              status: 'entity_resolved' as const,
              resolvedStateEntityId: existingStateEntityId,
              notes: { owner_name: item.ownerNameRaw, cycle: 'registry_key_reused' },
            };
            queryOutcomes.set(queryKey, outcome);
            await applyOwnerOutcome(client, item.ownerRowId, outcome);
            drilldownStats.drilldown_cycle_skipped_count += 1;
          ...
          if (expandedRegistryKeys.has(childRegistryKey)) {
            const outcome = {
              status: 'cycle_skipped' as const,
              notes: { owner_name: item.ownerNameRaw, registry_key: childRegistryKey },
            };
            queryOutcomes.set(queryKey, outcome);
            await applyOwnerOutcome(client, item.ownerRowId, outcome);
            drilldownStats.drilldown_cycle_skipped_count += 1;
          ...
          const childLookupKey = lookupKey(null, child.detail.legalName || item.ownerNameRaw);
          const childOwners = classifyOwnersForPersistence(ownerRowsForIowaDetail(child.detail), item.depth);
          const persistedChild = await persistIowaRegistryPull(
            client as unknown as Parameters<typeof persistIowaRegistryPull>[0],
            {
              companyId,
              lookupKey: childLookupKey,
              detail: child.detail,
              detailHtml: child.detailHtml ?? '',
              searchQuery: child.query,
              hitStatus: child.pick.hit?.status,
              owners: childOwners,
            },
          );

          expandedRegistryKeys.add(childRegistryKey);
          resolvedByRegistryKey.set(childRegistryKey, persistedChild.state_entity_id);
          const outcome = {
            status: 'entity_resolved' as const,
  ...
  await patchFoundryJobProgress(
    ...
    jobIdResolved,
    {
      ...
      companies_with_result: Object.values(reconciliationOutcomes).reduce((sum, n) => sum + Number(n || 0), 0),
      current_step: 'iowa_ecs_done',
      iowa_per_company: perCompany,
    },
  );

  console.log(
    JSON.stringify({ jobId: jobIdResolved, iowaCompanies: companyIds.length, perCompany: perCompany.length }),
  );
  logRec('worker-finished', {
    jobId: jobIdResolved,
    iowaCompanies: companyIds.length,
